...
DatabaseMySQL::DatabaseMySQL()
{
	...
	if (!mysql_real_connect(&m_handle, options.get<std::string>("sql.Host", "localhost").c_str(), options.get<std::string>("sql.User", "root").c_str(), options.get<std::string>("sql.Pass", "").c_str(), options.get<std::string>("sql.Db", "lotos2").c_str(), options.get<uint>("sql.Port", 3306), NULL, 0)){
