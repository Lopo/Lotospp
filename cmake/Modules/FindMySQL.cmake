...
#
#   MYSQL_DIR         - Set in environment or as parameter to "cmake", this is the top directory of the MySQL Server or
#                       Connector/C install
#   MYSQL_INCLUDE_DIR - Set in environment or as parameter to "cmake", this is the include directory where to find
#                       the client library
#   MYSQL_LIB_DIR     - Set in environment or as parameter to "cmake", this is the library directory where to find
#                       the client library
#   MYSQL_CLIENT_STATIC_LINKING
#                     - Specify that you want static linking, dynamic linking is the default
#   MYSQL_CLIENT_NO_THREADS
#                     - Specify to link against the single threaded library, "libmysqlclient". Note that in 5.5
#                       and up "libmysqlclient" is multithreaded and "libmysqlclient_r" just a soft link to it
...
#
# Note that most variables above, if not set by the user they will be set by this include file.
#
...
#   MYSQL_LIBRARIES   - Library name, might be "-lmysqlclient" while MYSQL_LIB is the path to the library
#
# (1) If MYSQL_INCLUDE_DIR or MYSQL_LIB_DIR are given, these are used and an error is reported if can't be used
# (2) If MYSQL_CONFIG_EXECUTABLE is given, it is used to get headers and libraries
# (3) If MYSQL_DIR is given and "${MYSQL_DIR}/bin/mysql_config" is found, then same as (2)
# (4) If MYSQL_DIR is given and no "${MYSQL_DIR}/bin/mysql_config", search MYSQL_DIR
#
...
# FIXME MYSQL_NUM_VERSION need to take into account Cluster versions and Connector/C versions
##########################################################################
...
foreach(_xvar ${ENV_OR_OPT_VARS})
	if ((DEFINED ${_xvar})
		...
		message(FATAL_ERROR "Please pass -D${_xvar}=... as an argument or set ${_xvar} in the environment, but not both")
	endif ()
	...
	# Normalize the path if the variable is set and is a path
	if (${_xvar})
...
set(_lib_fallback_path
	...
	$ENV{ProgramW6432}/MySQL/*/lib/${_lib_suffix_dist}
...
# For Windows, the client library name differs, so easy to make sure find_library() picks the right one. For Unix, it
# is the file extension that differs. In the static library case we know it is ".a", so we add it to the library name
...
message(STATUS "You will link ${_link_type}ally to the MySQL client library (set with -DMYSQL_CLIENT_STATIC_LINKING=<bool>)")
...
#       (change the info message if enabling this code)
...
elseif (MYSQL_DIR AND (NOT _mysql_config_in_mysql_dir) AND (NOT _mysql_config_set_by_user))
	if (FINDMYSQL_DEBUG)
...
elseif (MYSQL_CONFIG_EXECUTABLE)
	...
	# This code assumes that no space between "-I" and the path
	_mysql_config(MYSQL_INCLUDE_DIR_PATHS "(^| )-I" "--include")
	if (NOT MYSQL_INCLUDE_DIR_PATHS)
		message(FATAL_ERROR "Could not find the include dir from running \"${MYSQL_CONFIG_EXECUTABLE}\"")
	endif ()
	foreach (MID_PATH IN LISTS MYSQL_INCLUDE_DIR_PATHS)
		find_path(MYSQL_INCLUDE_DIR mysql.h ${MID_PATH})
		if (MYSQL_INCLUDE_DIR)
			break()
		endif ()
	endforeach ()
	if (NOT MYSQL_INCLUDE_DIR)
		message(FATAL_ERROR "Could not find \"mysql.h\" in \"${MYSQL_INCLUDE_DIR}\" found from running \"${MYSQL_CONFIG_EXECUTABLE}\"")
	endif ()
else ()
	if (FINDMYSQL_DEBUG)
		message("DBG: Using find_path() searching \"${_pp_include_fallback_path}\" to find \"mysql.h\"")
	endif ()
	...
	if (NOT _found_header)
		message(FATAL_ERROR "Could not find \"mysql.h\" from searching \"${_pp_include_fallback_path}\"")
	endif ()

	if (IS_DIRECTORY ${_found_header})
		set(MYSQL_INCLUDE_DIR ${_found_header})
	else ()
...
elseif (MYSQL_CONFIG_EXECUTABLE)
	...
	# This code assumes there is just one "-L...." and that no space between "-L" and the path
	_mysql_config(MYSQL_LIB_DIR_PATHS "(^| )-L" "--libs")
	if (NOT MYSQL_LIB_DIR_PATHS)
		message(FATAL_ERROR "Could not find the library dir from running \"${MYSQL_CONFIG_EXECUTABLE}\"")
	endif ()
	foreach (MLD_PATH IN LISTS MYSQL_LIB_DIR_PATHS)
		if (NOT EXISTS "${MLD_PATH}")
			message(WARNING "Could not find the directory \"${MLD_PATH}\" found from running \"${MYSQL_CONFIG_EXECUTABLE}\"")
			list(REMOVE_ITEM ${MYSQL_LIB_DIR_PATHS} ${MLD_PATH})
		endif ()
	endforeach ()
	list(LENGTH MYSQL_LIB_DIR_PATHS MLD_CNT)
	if (${MLD_CNT} EQUAL 0)
		message(FATAL_ERROR "Could not find the directory \"${MYSQL_LIB_DIR_PATHS}\" found from running \"${MYSQL_CONFIG_EXECUTABLE}\"")
	else ()
		set(MYSQL_LIB_DIR ${MYSQL_LIB_DIR_PATHS})
	endif ()
	...
	if (MYSQL_CLIENT_STATIC_LINKING)
		...
		# Replace the current library references with the full path to the library, i.e. the -L will be ignored
		_mysql_config_replace(MYSQL_LIBRARIES "(mysqlclient|mysqlclient_r)" "${MYSQL_LIB}" "(^| )-l" "--libs")
	else ()
	...
	if (FINDMYSQL_DEBUG)
		message("DBG: Using find_library() searching \"${_pp_lib_fallback_path}\" to find the client library")
	endif ()
	...
	if (NOT MYSQL_LIB)
		message(FATAL_ERROR "Could not find \"${_pp_search_libs}\" from searching \"${_pp_lib_fallback_path}\"")
	endif ()
...
##########################################################################
if (MYSQLCLIENT_STATIC_LINKING AND NOT ${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
